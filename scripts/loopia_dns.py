...
def klient() -> tuple[xmlrpc.client.ServerProxy, str, str]:
    env = env_read()
    anv = env.get("LOOPIA_API_USER", "")
    los = env.get("LOOPIA_API_PASSWORD", "")
    if not anv or not los:
        sys.exit(
            "LOOPIA_API_USER och LOOPIA_API_PASSWORD saknas i .env.deploy.\n"
            "Skapa en API-användare i Loopias kundzon under\n"
            "  Kontoinställningar -> LoopiaAPI\n"
            "och lägg in den. Det är den enda delen som kräver dig — se filens docstring."
        )
