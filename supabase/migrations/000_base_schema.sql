-- Bas-schemat: dashboardens tabeller (workspaces, profiles, business_contexts,
-- companies, contacts, campaigns …). Identiskt innehåll med supabase/schema.sql.
--
-- VARFÖR DEN HÄR FILEN FINNS: migrationskedjan var inte självbärande. 001 och
-- framåt förutsätter att workspaces redan finns, men den skapades bara av
-- schema.sql — som ingen migrationsmekanism kör. Följden märktes först när
-- databasgrenen för preview skapades: Supabase replayar `migrations/`, och
-- grenen fick ss_tenants (från 002) men INTE workspaces. Ett halvt schema som
-- såg ut att vara helt.
--
-- Samma lucka gäller varje framtida gren och varje återställning från noll.
--
-- Allt är `create table if not exists` / `on conflict do nothing`, så filen är
-- en no-op mot produktionen, där tabellerna redan finns.
...
drop policy if exists "members can manage own profile" on public.profiles;
create policy "members can manage own profile"
on public.profiles for select
using (workspace_id = public.current_workspace_id());

drop policy if exists "workspace scoped business contexts" on public.business_contexts;
create policy "workspace scoped business contexts"
on public.business_contexts for all
using (workspace_id = public.current_workspace_id())
with check (workspace_id = public.current_workspace_id());

drop policy if exists "workspace scoped companies" on public.companies;
create policy "workspace scoped companies"
on public.companies for all
using (workspace_id = public.current_workspace_id())
with check (workspace_id = public.current_workspace_id());

drop policy if exists "workspace scoped contacts" on public.contacts;
create policy "workspace scoped contacts"
on public.contacts for all
using (workspace_id = public.current_workspace_id())
with check (workspace_id = public.current_workspace_id());

drop policy if exists "workspace scoped lead signals" on public.lead_signals;
create policy "workspace scoped lead signals"
on public.lead_signals for all
using (workspace_id = public.current_workspace_id())
with check (workspace_id = public.current_workspace_id());

drop policy if exists "workspace scoped company insights" on public.company_insights;
create policy "workspace scoped company insights"
on public.company_insights for all
using (workspace_id = public.current_workspace_id())
with check (workspace_id = public.current_workspace_id());

drop policy if exists "workspace scoped campaigns" on public.campaigns;
create policy "workspace scoped campaigns"
on public.campaigns for all
using (workspace_id = public.current_workspace_id())
with check (workspace_id = public.current_workspace_id());

drop policy if exists "workspace scoped sequences" on public.sequences;
create policy "workspace scoped sequences"
on public.sequences for all
using (workspace_id = public.current_workspace_id())
with check (workspace_id = public.current_workspace_id());

drop policy if exists "workspace scoped sequence steps" on public.sequence_steps;
create policy "workspace scoped sequence steps"
on public.sequence_steps for all
using (workspace_id = public.current_workspace_id())
with check (workspace_id = public.current_workspace_id());

drop policy if exists "workspace scoped generated emails" on public.generated_emails;
create policy "workspace scoped generated emails"
on public.generated_emails for all
using (workspace_id = public.current_workspace_id())
with check (workspace_id = public.current_workspace_id());

drop policy if exists "workspace scoped email events" on public.email_events;
create policy "workspace scoped email events"
on public.email_events for all
using (workspace_id = public.current_workspace_id())
with check (workspace_id = public.current_workspace_id());

drop policy if exists "workspace scoped meetings" on public.meetings;
create policy "workspace scoped meetings"
on public.meetings for all
using (workspace_id = public.current_workspace_id())
with check (workspace_id = public.current_workspace_id());

drop policy if exists "workspace scoped suppressions" on public.suppressions;
create policy "workspace scoped suppressions"
on public.suppressions for all
using (workspace_id = public.current_workspace_id())
with check (workspace_id = public.current_workspace_id());

drop policy if exists "workspace scoped audit logs" on public.audit_logs;
