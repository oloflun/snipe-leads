...
def _render_service() -> dict[str, str]:
    """Platt nyckel->värde ur render.yamls första tjänst.

    Handrullad i stället för PyYAML: invariantsviten körs med enbart
    snajp-support/requirements.txt installerat, och att lägga till ett
    beroende för fyra rader vore fel avvägning. Filen är vår egen och
    formatet är stabilt.
    """
    values: dict[str, str] = {}
...
def test_build_context_is_the_repo_root():
    ...
    assert service.get("rootDir") == ".", (
        f"render.yaml rootDir={service.get('rootDir')!r}, måste vara '.'. "
    ...
    assert service.get("dockerfilePath") == "./snajp-support/Dockerfile", (
        "dockerfilePath måste peka på snajp-support/Dockerfile relativt "
        f"repo-roten, fick {service.get('dockerfilePath')!r}."
    )
