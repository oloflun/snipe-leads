...
| `leads`, `support` (development; main provisioneras med `scripts/railway_agentsajt.py --tjanst <x> --env main --apply` vid release) | Fristående agentsajter ur `leads-webb/` resp. `support-webb/` (2026-09-15). Samma mönster som bokforing: vänsterrail, /logga-in (`AGENTSAJT_EPOST`/`AGENTSAJT_LOSEN`), SSO-kundsession, generisk tenant-proxy under `/api/ag`. Domäner: `leads-development` / `support-development.up.railway.app`. "Kör Agent"-knapparna i webbens flikar styrs av `LEADS_EXTERN_URL` / `SUPPORT_EXTERN_URL` på web-tjänsten (osatta i main tills sajterna provisioneras där) |
