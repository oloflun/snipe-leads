...
| `bokforing`, `leads` | **Bortlagda ur koden 2026-09-19.** `bokforing-webb/` och `leads-webb/` veks in i huvudappen (Kvitton och Iris under `/dashboard`). Miljövariablerna `BOKFORING_EXTERN_URL`, `BOKFORING_SSO_SECRET` och `LEADS_EXTERN_URL` är borttagna från `web` i båda miljöerna (2026-09-19). Railway-tjänsterna `bokforing`, `leads` och dubbletten `bokforing-4f55535c-…` i main är ännu INTE raderade — Antons steg. |
| `support` (båda miljöerna) | Fristående agentsajt ur `support-webb/` (leads-sajten bortlagd 2026-09-19, se raden ovan) — development 2026-09-15, main 2026-09-16 via `scripts/railway_agentsajt.py --tjanst <x> --env main --apply`. Samma mönster som bokforing: vänsterrail, /logga-in (`AGENTSAJT_EPOST`/`AGENTSAJT_LOSEN`), SSO-kundsession, generisk tenant-proxy under `/api/ag`. Domäner: `leads-development` / `leads-main` och `support-development` / `support-main.up.railway.app`. "Kör Agent"-knapparna i webbens flikar styrs av `LEADS_EXTERN_URL` / `SUPPORT_EXTERN_URL` på web-tjänsten, satta i båda miljöerna | Står kvar tills Anton och Sebbe avgjort om supportportalen ska vika in i huvudappen som de andra två. |
